...
dificultad_tot <- dificultad_tot %>%
  mutate(
    porc_dif1 = as.numeric(str_replace(porc_dif1, "%", "")),
    total = as.numeric(str_replace(total, "%", ""))
  )

dificultad_constru <- dificultad_constru %>%
  mutate(
    porc_dif1 = as.numeric(str_replace(porc_dif1, "%", "")),
    total = as.numeric(str_replace(total, "%", ""))
  )
...
ggplot(dificultad_larga, aes(fill = sector, y = valor, x = dificultad)) + 
  geom_bar(position = "dodge", stat = "identity", color = "white", size = 0.1) + 
  scale_fill_manual(values = c("Módulo Agrícola" = "#64C3D2", 
                               "Módulo Construcción" = "#005691", 
                               "Módulo General" = "#002A51")) +
  coord_flip() +
  theme_classic() +
  geom_text(aes(label = paste0(valor, "%")), 
            position = position_dodge(width = 0.9), 
            vjust = 0.5, hjust = -0.3, color = "#002A51", size = 2) +  
