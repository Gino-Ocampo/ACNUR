...
format:
  pdf:
    ...
    document:
      tables:
        float: false
    extra_dependencies: ["pdfpages", "float"]  # pdfpages se cargará correctamente
...
vacantes_tot <- readxl::read_xlsx("../Tablas/porcentajes.xlsx", sheet = "vacantes_tot")
...
vacantes_tot<- vacantes_tot %>% mutate(cv= paste0(cv, "%"))
...
ggplot(dificultad_larga, aes(fill = sector, y = valor, x = dificultad)) + 
  geom_bar(position = "dodge", stat = "identity", color = "white", size = 0.5, width =0.9) + 
...
unique(conocimiento_2$programa)
...
vacantes_tot <- readxl::read_xlsx("../Tablas/porcentajes.xlsx", sheet = "vacantes_tot") %>% select(oficio_label,vacantesu12, cv)


vacantes_tot <- vacantes_tot %>%  group_by(oficio_label) %>%
  mutate(vacantesu12 = sum(vacantesu12, na.rm = TRUE),  # Suma de vacantes
    cv = sum(cv * vacantesu12, na.rm = TRUE) / sum(vacantesu12, na.rm = TRUE)) %>%
  summarise(oficio_label,vacantesu12=round(vacantesu12,0),cv=round(cv*100,1)) %>% 
  arrange(desc(vacantesu12)) 

vacantes_tot<-vacantes_tot %>% group_by(oficio_label) %>% summarise(vacantesu12=mean(vacantesu12, na.rm=T),
                                                      cv=mean(cv, na.rm=T))%>% arrange(desc(vacantesu12))
