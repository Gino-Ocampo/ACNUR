...
# cargar enadel
enadel_wide  <- read_dta("data/clean/enadel_wide.dta") |> subset(region_trab%in%region) |> 
  mutate(empresas = 1,
         estrato_cm = as.integer(interaction(p6, a2, tam_val)),
         estrato_rt = as.integer(interaction(region_trab, a2, tam_val)))

enadel_long  <- read_dta("data/clean/enadel_long.dta") |> subset(region_trab%in%region) |> 
  mutate(oficios = 1, 
